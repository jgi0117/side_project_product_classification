...
| 모델 | computer 재현율 k=1 → 2 | computer FPR k=1 → 2 | computer FNR k=1 → 2 | book 재현율 k=1 → 2 | book FPR k=1 → 2 | book FNR k=1 → 2 |
|---|---:|---:|---:|---:|---:|---:|
| tiny | 89.00% → 97.50% | 1.74% → 11.63% | 11.00% → 2.50% | 67.61% → 98.59% | 0.33% → 23.92% | 32.39% → 1.41% |
| s | 91.50% → 98.00% | 1.16% → 15.70% | 8.50% → 2.00% | 77.46% → 98.59% | 0.00% → 20.27% | 22.54% → 1.41% |
| m | 94.50% → 99.50% | 0.00% → 13.37% | 5.50% → 0.50% | 78.87% → 98.59% | 0.00% → 17.94% | 21.13% → 1.41% |
| l | 94.50% → 99.00% | 0.58% → 12.21% | 5.50% → 1.00% | 77.46% → 97.18% | 0.00% → 18.60% | 22.54% → 2.82% |
| x | 94.50% → 98.50% | 0.00% → 8.14% | 5.50% → 1.50% | 85.92% → 100.00% | 0.33% → 14.62% | 14.08% → 0.00% |

FNR은 실제 해당 카테고리 이미지 중 후보에 포함되지 않은 비율로, 같은 카테고리의 `100% − 재현율`입니다. FPR은 다른 카테고리 이미지가 후보로 잘못 포함된 비율입니다.
